define([
    ...
    function CesiumWidget(options) {
        ...
        widgetElement.className = 'cesium-widget';
        //TS-GIS
        ...
        this._tooltipContainer.className = 'overlay-container';
        element.appendChild(this._tooltipContainer);
        this._tooltipStopEventContainer = document.createElement('div');
        this._tooltipStopEventContainer.className = 'ol-overlaycontainer-stopevent';
        widgetElement.appendChild(this._tooltipStopEventContainer);

        //TS-GIS
        this._popupWindows = [];
        this._popupWindowContainer = document.createElement('div');
        this._popupWindowContainer.className = 'overlay-container';
        element.appendChild(this._popupWindowContainer);
        ...
        try {
            ...
            this._screenSpaceEventHandler = new ScreenSpaceEventHandler(element, false);
            ...
            //TS-GIS
            this._scene.camera.changed.addEventListener(function (e) {
                var a = e;
            },this);
        } catch (error) {
...
//TS-GIS
    CesiumWidget.prototype.on = function (name, func) {
        if (name in this._events) {
            var events = this._events[name];
            if(events.indexOf(func) < 0)
                events.push(func);
        }
    }
    //TS-GIS
    CesiumWidget.prototype.un = function (name, func) {
        if (name in  this._events) {
            var events = this._events[name];
            var index = events.indexOf(func);
            if(index >= 0)
                events.splice(index, 1);
        }
    }
    //TS-GIS
    CesiumWidget.prototype.setTarget = function(container){
        this._container = container;
    ...
    //TS-GIS
    CesiumWidget.prototype.addOverlay = function (overlay) {
        if(this._overlays.indexOf(overlay) == -1){
            if(!!overlay.stopEvent){
                this._overlays.push(overlay);
                this._tooltipStopEventContainer.appendChild(overlay.container);
            }
            else {
                this._overlays.push(overlay);
                this._tooltipContainer.appendChild(overlay.container);
            }
        }
    }

    CesiumWidget.prototype.removeOverlay = function (overlay) {
        var index = this._overlays.indexOf(overlay);
        if( index>= 0){
            if(!!overlay.stopEvent){
                this._tooltipStopEventContainer.removeChild(overlay.container);
            }
            else {
                this._tooltipContainer.removeChild(overlay.container);
            }
            this._overlays.splice(index,1);
        }
    }
    //TS-GIS
    CesiumWidget.prototype.addPopup = function (popup) {
        if(this._popupWindows.indexOf(popup) == -1){
            this._popupWindows.push(popup);
            this._popupWindowContainer.appendChild(popup.container);
        }
    }

    CesiumWidget.prototype.removePopup = function (popup) {
        var index = this._popupWindows.indexOf(popup);
        if( index>= 0){
            this._popupWindowContainer.removeChild(popup.container);
    ...
    CesiumWidget.prototype.render = function() {
        if (this._canRender) {
            ...
            //TS-GIS
            var events = this._events['render'];
            if(!!events){
