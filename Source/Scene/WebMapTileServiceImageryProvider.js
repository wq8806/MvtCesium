define([
    ...
    function requestImage(imageryProvider, col, row, level, request, interval) {
        ...
        //TS-GIS
        var tileMatrix = defined(labels)  && level < labels.length? labels[level] : level.toString();
        ...
        if (!imageryProvider._useKvp) {
            resource._url = resource._url.replace('{s}', subdomains[(col + row + level) % subdomains.length]);
        }
        //TS-GIS
        /*var resource;
        if (!imageryProvider._useKvp) {
