define([
    ...
    PrimitiveCollection.prototype.remove = function (primitive) {
        ...
        if (this.contains(primitive)) {
            ...
            if (index !== -1) {
                ...
                //TS-GIS
                if(primitive instanceof WaterEffect){
                    var scene = primitive._scene;
                    var waters = scene.frameState.water;
                    for(var i = waters.length - 1;i >=0;i--){
                        if(waters[i]._id === primitive._id){
                            waters.splice(i,1);
                        }
                    }
                }
    ...
    PrimitiveCollection.prototype.removeAll = function () {
        ...
        for (var i = 0; i < length; ++i) {
            ...
            if (this.destroyPrimitives) {
                //TS-GIS
                if(primitives[i] instanceof WaterEffect){
                    var scene = primitives[i]._scene;
                    var waters = scene.frameState.water;
                    waters.length = 0;
                }
    ...
    PrimitiveCollection.prototype.raise = function (primitive) {
        if (defined(primitive)) {
            ...
            var primitives = this._primitives;
            //TS-GIS
            if (index >= 0 && index !== primitives.length - 1) {
