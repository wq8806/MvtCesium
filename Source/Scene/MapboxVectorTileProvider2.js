...
define(['../Core/defaultValue','../Core/defined','../Core/DeveloperError','../Core/WebMercatorTilingScheme','../Core/GeographicTilingScheme','../ThirdParty/when','../ThirdParty/ol-debug','../ThirdParty/mapbox-style','../Scene/TileReplacementQueue','../Core/defineProperties','../Core/Resource'],
    ...
    var MVTProvider = function (options) {
        ...
        this._projection = ol.proj.get('EPSG:4326');
        this._projection.setExtent([-180,-90,180,90]);
        ...
        /*fetch(url).then(function (response) {
            //if (response.status === 200) {
                response.arrayBuffer().then(function (data) {
                    var features1 = mvtParser.readFeatures(data);
                    done(features1);
                }).catch(function () {
                    done();
                });
        ...
        if(cacheTile != undefined){
            ...
            /*var tilerequest = */return function(x,y,z){
                ...
                //return resource.fetchArrayBuffer().then(function(arrayBuffer) {
                    ...
                    /*debugger
                    that._layerRenderer.getTile(z,x,y,window.devicePixelRatio,that._projection);*/
                    ...
                    var done = function(fs) {
                        ...
                        if ( count === 1) {
                            for(var i=0;i<features.length;i++){
                                ...
                                // that._layerRenderer.renderFeature(feature,squaredTolerance,styles,_replayGroup);
                    ...
                    getFeaturesByUrl(url.replace("{index}", 11), done);
                    // getFeaturesByUrl(url.replace("{index}", 13), done);
                    // getFeaturesByUrl(url.replace("{index}", 16), done);
